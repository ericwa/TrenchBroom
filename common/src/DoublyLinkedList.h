...
public:
    ...
    private:
        int compare(const iterator_base& other) const {
            ensure(m_delegate != NULL, "m_delegate is null");
            ensure(other.m_delegate != NULL, "other.m_delegate is null");
...
public:
    ...
    void insertBefore(Item* succ, Item* items, const size_t count) {
        ensure(succ != NULL, "succ is null");
        ensure(items != NULL, "items is null");
        ensure(m_head != NULL, "m_head is null");
